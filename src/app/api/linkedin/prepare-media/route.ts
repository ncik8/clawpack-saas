...
export async function POST(request: Request) {
  try {
    ...
    console.log('LinkedIn prepare-media register:', registerRes.status, JSON.stringify(registerData));
    ...
    if (!uploadUrl) {
      return NextResponse.json({ error: 'No upload URL in response' }, { status: 400 });
    }
    ...
    console.log(`LinkedIn prepare-media: fetched image ${imageBuffer.length} bytes, type=${contentType}`);

    // Step 3: PUT image bytes to LinkedIn upload URL - use exact URL as returned
    ...
    console.log(`LinkedIn prepare-media upload PUT: ${uploadRes.status} ${uploadRes.statusText}`);

    if (!uploadRes.ok) {
      const errBody = await uploadRes.text();
      console.error('LinkedIn prepare-media upload failed:', errBody);
      return NextResponse.json({ 
        error: 'LinkedIn image upload failed',
        status: uploadRes.status,
        body: errBody.substring(0, 300)
      }, { status: 400 });
    }

    console.log(`LinkedIn prepare-media: SUCCESS - asset=${assetUrn}`);
    return NextResponse.json({
      success: true,
      assetUrn,
    });
    ...
    console.error('LinkedIn prepare-media error:', error);
