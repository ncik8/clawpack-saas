...
const N8N_URL = process.env.NEXT_PUBLIC_N8N_URL || 'http://159.223.42.131:5678';
...
export default function ConnectedAccountsPage() {
  ...
  const handleConnect = (platformId: string) => {
    // Open n8n in new tab for authorization
    window.open(N8N_URL, '_blank');
    setConnecting(platformId);
    
    // For demo: mark as connected after 3 seconds (simulating OAuth callback)
    setTimeout(() => {
    ...
    <div className="p-8">
            ...
            <div key={platform.id} className="card hover:border-[#1780e3]/50 transition-colors">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-[#9ca3af] mt-1">{platform.description.replace(/'/g, "'")}</p>
                </div>
              </div>
      ...
      <div className="mt-8 p-4 rounded-lg bg-[#1780e3]/10 border border-[#1780e3]/30">
        <p className="text-sm text-[#1780e3]">
          <strong>💡 How it works:</strong> Clicking Connect opens n8n where you authorize 
          the account. Once authorized, you can manage all your social accounts from one place 
